...
use crossterm::{event, execute, terminal, cursor, queue, style::{*, self}};
use std::io::stdout;
use std::io::Write;
use std::io::*;
...
// Implement search index for incremental search
impl SearchIndex {
    fn new() -> Self {
        Self {
            x_index: 0,
    ...
    // Resets search index values back to default
    fn reset(&mut self) {
        self.y_index = 0;
        self.x_index = 0;
...
// StatusMessage struct implementation
impl StatusMessage {
...
impl EditorRows {
    ...
    fn join_adjacent_rows(&mut self, at: usize) {
        ...
        let previous_row = self.get_editor_row_mut(at);
    ...
    fn render_row(row: &mut Row) {
        ...
        row.row_content.chars().for_each(|c| {
            index += 1;
            // Check if character in string is a tab and render it as such
            if c == '\t' {
                row.render.push(' ');
...
impl Output {
    ...
    fn draw_message_bar(&mut self) {
        ...
        if let Some(msg) = self.status_message.message() {
            ...
            let msg_len = cmp::min(msg.len(), self.win_size.0);

            for _ in msg_len..self.win_size.0 {
    ...
    // Draw status bar at botom of screen
        ...
        for i in 0..screen_rows {
            ...
            if file_row >= self.editor_rows.number_of_rows() {
                if self.editor_rows.number_of_rows() == 0 && i == screen_rows / 3 {
                    let mut welcome = format!("Aurora Editor --- Version 1.0");
                    if welcome.len() > screen_columns {
                        welcome.truncate(screen_columns);
                    }
                    let mut padding = (screen_columns - welcome.len()) / 2;
                    if padding != 0 {
                        self.editor_contents.push_str(&self.line_char);
                        padding -= 1;
                    }
    ...
    fn insert_newline(&mut self) {
        if self.cursor_controller.cursor_x == 0 {
            ...
            let current_row = self
                .editor_rows
                .get_editor_row_mut(self.cursor_controller.cursor_y);
            let new_row_content = current_row.row_content[self.cursor_controller.cursor_x..].into();            current_row
                .row_content
...
// Disables raw mode for terminal
